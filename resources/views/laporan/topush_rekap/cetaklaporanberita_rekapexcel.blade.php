            ...
            @if($satker->roles!='superadmin')
                @php
                    $total_website = DB::select(DB::raw("SELECT count(website) as jml_website from berita
                        where kode_satker='$satker->kode_satker' and length(website)>10 AND tgl_input between '$dari' and '$sampai'"));

                $total_facebook = DB::select(DB::raw("SELECT count(facebook) as jml_facebook from berita
                    where kode_satker='$satker->kode_satker' and length(facebook)>10 AND tgl_input between '$dari' and '$sampai'"));

                    $total_instagram = DB::select(DB::raw("SELECT count(instagram) as jml_instagram from berita
                    where kode_satker='$satker->kode_satker' and length(instagram)>10 AND tgl_input between '$dari' and '$sampai'"));

                $total_twitter = DB::select(DB::raw("SELECT count(twitter) as jml_twitter from berita
                    where kode_satker='$satker->kode_satker' and length(twitter)>10 AND tgl_input between '$dari' and '$sampai'"));
                $total_tiktok = DB::select(DB::raw("SELECT count(tiktok) as jml_tiktok from berita
                    where kode_satker='$satker->kode_satker' and length(tiktok)>10 AND tgl_input between '$dari' and '$sampai'"));
                $total_sippn = DB::select(DB::raw("SELECT count(sippn) as jml_sippn from berita
                    where kode_satker='$satker->kode_satker' and length(sippn)>10 AND tgl_input between '$dari' and '$sampai'"));
                $total_youtube = DB::select(DB::raw("SELECT count(youtube) as jml_youtube from berita
                    where kode_satker='$satker->kode_satker' and length(youtube)>10 AND tgl_input between '$dari' and '$sampai'"));
                @endphp
                <tr>
                    @php
